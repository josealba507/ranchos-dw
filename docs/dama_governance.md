...
| **Accuracy** (exactitud) | `tests/exactitud_existencia_insumo_vs_ledger.sql` — reconcilia el ledger inmutable de `movimientos_insumos` (`SUM(cantidad_base)`, agrupado por insumo) contra `existencia_actual` del catálogo (el saldo cacheado que el cliente incrementa atómicamente vía `FieldValue.increment`). Mismo principio que los scripts `reconciliar-*.js` del proyecto operacional, pero como test automatizado en vez de script manual de un solo uso — primera instancia concreta de esta dimensión, ver `docs/fase5_exactitud_insumos.md`. |
| **Detección de anomalías** | Paquete `elementary-data/elementary` (ver `docs/fase5_anomalias_elementary.md`) — 3 tests (`volume_anomalies`/`freshness_anomalies`/`all_columns_anomalies`) sobre las 26 tablas de `sources.yml`, comparando contra el patrón histórico real acumulado en corridas sucesivas de `dbt test`, no contra umbrales fijos. Complementa (no reemplaza) la reconciliación exacta y el `source freshness` de umbral fijo de arriba — cubre desviaciones estadísticas sutiles que un umbral fijo no anticiparía. **Sin historial acumulado todavía no detecta anomalías reales** — depende de que se automaticen corridas regulares de dbt (ítem pendiente en "Prioridades actuales" de `CLAUDE.md`). |
